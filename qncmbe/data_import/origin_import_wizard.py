...
class ImportFrame(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def set_default_file(self):
        ...
        self.file.setText(f"Origin_data_{start_str}_to_{end_str}.opj")
    ...
    def import_data(self):
        ...
        if filename == "":
            self.runtime_messages.appendPlainText(
                "Error: filename is missing. Check before running again."
            )
            return
        ...
        try:
            ...
            self.runtime_messages.appendPlainText(
                "Error: invalid time step. Check before running again."
            )
            return
        ...
        if t_step == 0:
            ...
            self.runtime_messages.appendPlainText(
                "Special case detected! (Time step = 0) Generating null data."
            )
            self.runtime_messages.repaint()
        elif t_step <= 0.0:
            self.runtime_messages.appendPlainText(
                "Error: time step must be positive. "
                "Change before running again"
            )
            return
        ...
        try:
            self.runtime_messages.appendPlainText(
                f'Loading template "{self.default_template_file}"...'
            )
            self.runtime_messages.repaint()
        ...
        try:
            ...
            for loc in locs:
                ...
                for n in range(ncols):
                    ...
                    origin.Execute(f'col({n+1})[L]$ = {name}')

                    # Add units to each column
                    origin.Execute(f'col({n+1})[U]$ = {units}')
        ...
        try:
            ...
            self.runtime_messages.appendPlainText(
                "Error: could not save output file. Save manually if possible."
            )
            del origin
            return
        ...
        # Stop running Origin in the background (release control of the 
        # Origin file to the user)
        del origin 
