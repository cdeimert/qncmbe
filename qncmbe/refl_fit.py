'''
Functions for fitting reflectance oscillations in Molecular Beam Epitaxy
growths. Based on the method of Breiland et al ("A virtual interface method for
extracting growth rates and high temperature optical constants from thin
semiconductor films using in situ normal incidence reflectance", Journal of
Applied Physics, 1995)
...
def fit_reflectance(refl, time, pars_guess):
    ...
    - pars_guess should be a dictionary of FittableParameter objects with keys
      equal to {'n', 'k', 'ns', 'ks', 'G', 's', 'wvln'}
    '''
    ...
    def refl_func(t, *p_arr):
        return calc_reflectance(
            t, **{k: p for k, p in zip(pfit_keys, p_arr)}, **pfix
        )
...
def print_fitted_value(name, x, units='', print_error=True):
    ...
    if x.is_fitted:
        ...
        if (np.abs(x.val0) > 1e-20) and print_error:
            ...
            string = f'{name} = {x.valopt:.5f}{pad_units} '\
                     f'({x_err:.5f} % away from initial guess of '\
                     f'{x.val0:.5f}{pad_units})'
        else:
            string = f'{name} = {x.valopt:.5f}{pad_units} '\
                     f'(Initial guess: {x.val0:.5f}{pad_units})'

    else:
        string = f'{name} = {x.val0:.5f}{pad_units} (Fixed)'
...
class Layer():
    def __init__(
        self, material, material_beneath, growth_rate,
        t_start=-np.inf, t_end=np.inf
    ):
        '''
        - material and material_beneath should be of class Material
        - growth_rate should be a float (if using angtroms, should call
          use_angstroms_for_structure())
        - If t_start and t_end are specified, will automatically use them to
          restrict the data provided in set_refl_data()
        '''
    ...
    def set_refl_data(self, t, R):
        ...
        R should be a dictionary of numpy arrays. One entry for
        each wavelength.
        '''
    ...
    def set_refl_pars_guess(self, fit_pars='ns,ks,G,s'):
        ...
        for wvln in self.refl_data:
            ...
            self.refl_pars_guess[wvln] = {
                k: FittableParameter(vals_dict[k]) for k in vals_dict}
    ...
    def calc_refl_fit(self):
        ...
        for wvln in self.refl_data:
            self.refl_pars_fit[wvln], self.refl_fit[wvln] = fit_reflectance(
                self.refl_data[wvln], self.t_data, self.refl_pars_guess[wvln]
            )
    ...
    def print_refl_fit(self):
        ...
        for wvln in self.refl_fit:
            ...
            for key in ['n', 'k', 'ns', 'ks']:
                string += print_fitted_value(key,
                                             pars_fit[key], print_error=False)
    ...
    def plot_refl_fit(self):
        ...
        for wvln in self.refl_data:
            if wvln in wvln_colors_light:
                ...
                color = f'C{n}'
                n += 1
            ax.plot(self.t_data, self.refl_data[wvln],
                    '-', label=wvln + " nm data", color=color)
            ax.plot(self.t_data,
                    self.refl_fit[wvln], ':k', label=wvln + " nm fit")
...
class Structure():
    ...
    def plot_full_refl_data(self):
        ...
        for wvln in self.R_data:
            if wvln in wvln_colors_dark:
                ...
                color = f'C{n}'
                n += 1
            ax.plot(self.t_data, self.R_data[wvln],
                    '-', label=wvln + " nm", color=color)
    ...
    def add_layer(
        ...
        new_layer = Layer(material, material_beneath,
                          growth_rate, t_start, t_end)

        if name in self.layer_names:
            raise Exception(
                f'Layer name "{name}" already used. '
                f'Each layer must have a unique name.'
            )
    ...
    def get_fit_convergence(self, layer_name, t_buffer, t_step, parameter='G'):
        ...
        while (t <= t_end) and (t <= self.t_data[-1]):
            ...
            for wvln in self.R_data:
                p_out[wvln].append(pfit[wvln][parameter].valopt)
    ...
    def plot_fit_convergence(self, layer_name, t_buffer, t_step, parameter):
        ...
        t, p = self.get_fit_convergence(
            layer_name, t_buffer, t_step, parameter)
