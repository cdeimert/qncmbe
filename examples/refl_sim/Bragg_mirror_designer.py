...
In this case, used to design a repeating AlAs/GaAs Bragg mirror on a
GaAs substrate.

This uses a simple transfer matrix implementation, from the package
qncmbe.refl_sim
'''
...
# Note, for ideal Bragg reflection, the layers should be n/2+1/4
# wavelengths thick. On the other hand, a thickness of n/2 will make the
# layer invisible.
...
for layer in struct.layers:
    ...
    rect = patches.Rectangle(
        ([layer.t[0], 0]), layer.t[-1] - layer.t[0], 1.0,
        facecolor=color, lw=0.0
    )
    ax.add_patch(rect)
...
for wavelength in wavelengths:
    wl = str(wavelength)
    ax.plot(t[wl], R[wl], label=f'{wl} nm', color=colors[wl])
